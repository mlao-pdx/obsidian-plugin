...
| Internal operation                          | **Normal** for Realm-rooted Islands, and for narrative-scope resolution _within_ a non-Realm-rooted Island's own subtree (Book/Series boundaries still resolve normally relative to each other inside it) — but a non-Realm-rooted Island as a whole still has no `realmId` and is therefore never materialized as an indexed row, never compiled, never reported outside the single structure-issues line below.                                      |
...
| `realmId`, Island **not** rooted in a Realm | **None, permanently.** Crossing an Island boundary is forbidden in both directions (§5.3), so a `realmId` can never be reached from outside and none exists inside. **This Island is a headless orphan**: no Canonical Index row, no scope (§5.2's "no Realm reached" case applies), invisible to every Narradin operation. Its only trace is the structure-issues report, produced by the pre-scope folder-tree scan (§4.2), not a scope-bound query. |
